...
- **Wrangler v4 action deployment**: Update release workflow to `cloudflare/wrangler-action@v4` with `packageManager: bun` so static asset routing (`run_worker_first`) and headers are processed by current Wrangler 4 instead of legacy Wrangler 3.90.0.

## [1.3.1] - 2026-09-11

### Fixed

- **Production `/api/live` endpoint**: Emit build-time static `/api/live` health JSON (`status: "ok"`, `name: "matrix"`, current package version) and configure `public/_headers` with `Cache-Control: no-store` so production returns standard JSON instead of SPA fallback HTML.
