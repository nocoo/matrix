...
export default function InteractionShowcasePage() {
	const { toasts, dialogs, variantLabels, activeDialog, openDialog, closeDialog, getDialogById } =
	...
	return (
		<div className="space-y-4">
			...
			{currentDialog && (
				<>
					<div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm" onClick={closeDialog} />
					<div className="fixed inset-x-4 top-[25%] z-50 mx-auto max-w-md">
						<AsciiBox title={currentDialog.title.toUpperCase()}>
							<div className="space-y-3">
								<p className="font-mono text-xs text-matrix-muted">{currentDialog.description}</p>
								{currentDialog.style === "form" && (
									<div className="space-y-2">
										<MatrixInput label="feedback" type="text" placeholder="type your feedback..." />
									</div>
								)}
								<div className="flex justify-end gap-2">
									<MatrixButton onClick={closeDialog}>[{t("common.cancel")}]</MatrixButton>
									<MatrixButton onClick={closeDialog}>
										{currentDialog.style === "confirm"
											? `[${t("common.confirm")}]`
											: `[${t("common.ok")}]`}
									</MatrixButton>
								</div>
