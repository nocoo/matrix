...
import { AsciiBox, DataRow, MatrixAvatar } from "@/components/ui";
		...
		<div className="space-y-4">
			<AsciiBox title="DATA DISPLAY" subtitle="passive content">
				<p className="text-xs font-mono text-matrix-muted">
					Avatars, badges, stat tiles, timeline, list items, and key-value displays — all passive
					data presentation patterns in Matrix style.
				</p>
			</AsciiBox>
					...
					<div>
						...
						<div className="flex items-center gap-4">
							<div className="flex flex-col items-center gap-1">
								<div className="w-10 h-10 border border-matrix-ghost bg-matrix-panel flex items-center justify-center font-mono text-xs text-matrix-muted">
									NE
								</div>
								<span className="text-[10px] font-mono text-matrix-dim">Plain</span>
							</div>
							<div className="flex flex-col items-center gap-1">
								<div className="w-10 h-10 border border-matrix-primary/30 bg-matrix-primary/10 flex items-center justify-center font-mono text-xs text-matrix-primary">
									MO
								</div>
								<span className="text-[10px] font-mono text-matrix-dim">Primary</span>
							</div>
							<div className="flex flex-col items-center gap-1">
								<div className="w-10 h-10 border border-green-500/30 bg-green-500/10 flex items-center justify-center font-mono text-xs text-green-400">
									TR
								</div>
								<span className="text-[10px] font-mono text-matrix-dim">Green</span>
							</div>
							<div className="flex flex-col items-center gap-1">
								<div className="w-10 h-10 border border-yellow-500/30 bg-yellow-500/10 flex items-center justify-center font-mono text-xs text-yellow-400">
									OR
								</div>
						...
						<div className="flex items-center">
							{["neo", "morpheus", "trinity", "oracle", "tank"].map((name, i) => (
								<div
									...
									style={{ zIndex: 5 - i, marginLeft: i === 0 ? 0 : -8 }}
								>
									<MatrixAvatar
										name={name}
										size={32}
										className="border-2 border-[var(--matrix-bg)]"
									/>
								</div>
							))}
							<div
								className="w-8 h-8 border-2 border-[var(--matrix-bg)] bg-matrix-panel-strong flex items-center justify-center font-mono text-[10px] text-matrix-dim"
				...
				<div className="space-y-4">
					<div>
						...
						<div className="flex flex-wrap gap-2">
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase bg-matrix-primary text-black">
								DEFAULT
							</span>
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase bg-matrix-panel-strong text-matrix-muted border border-matrix-ghost">
								SECONDARY
							</span>
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase bg-red-500 text-black">
								DESTRUCTIVE
							</span>
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase border border-matrix-ghost text-matrix-muted">
								OUTLINE
							</span>
						</div>
					</div>
					...
					<div>
						...
						<div className="flex flex-wrap gap-2">
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase bg-green-500/15 text-green-400 border border-green-500/30">
								ACTIVE
							</span>
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase bg-yellow-500/15 text-yellow-400 border border-yellow-500/30">
								PENDING
							</span>
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase bg-red-500/15 text-red-400 border border-red-500/30">
								FAILED
							</span>
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase bg-blue-500/15 text-blue-400 border border-blue-500/30">
								INFO
							</span>
							<span className="px-2 py-0.5 text-[10px] font-mono font-bold uppercase bg-purple-500/15 text-purple-400 border border-purple-500/30">
								BETA
							</span>
						</div>
					</div>
					...
					<div>
						<p className="text-[10px] font-mono text-matrix-dim uppercase mb-3">
							With dot indicator
						</p>
						<div className="flex flex-wrap gap-3">
							{[
								...
								<span
									key={b.label}
									className="flex items-center gap-1.5 px-2 py-0.5 text-[10px] font-mono font-bold uppercase border border-matrix-ghost"
								>
					...
					{vm.timeline.map((event, i) => (
						<div key={event.id} className="flex gap-3 pb-4 last:pb-0">
							...
							<div className="flex flex-col items-center">
								...
								/>
								{i < vm.timeline.length - 1 && <div className="w-px flex-1 bg-matrix-ghost mt-1" />}
							</div>
							{/* Content */}
							<div className="pb-2">
				...
				<Section title="LIST ITEMS">
					<div className="space-y-0">
						{vm.people.map((person) => (
							<div
								key={person.name}
								className="flex items-center gap-3 py-2 border-b border-matrix-ghost last:border-b-0 hover:bg-matrix-panel transition-colors px-2"
							>
								<MatrixAvatar name={person.name} size={28} />
								<div className="flex-1 min-w-0">
									<p className="text-xs font-mono text-matrix-primary font-bold uppercase truncate">
										{person.name}
									</p>
								...
								>
									<span
										className={cn(
											...
											person.status === "active" ? "bg-green-400" : "bg-matrix-dim",
										)}
									/>
									{person.status}
								</span>
