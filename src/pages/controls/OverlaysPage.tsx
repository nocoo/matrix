...
import { AsciiBox, FloatingPortal, MatrixButton, MatrixInput } from "@/components/ui";
...
function Sheet({
	...
	return (
		<>
			...
			<div
				className={cn(
					"fixed z-50 matrix-panel border-matrix-primary/30 p-5 overflow-y-auto matrix-scrollbar",
		...
		<div className="space-y-4">
			<AsciiBox title="OVERLAYS" subtitle="layered surfaces">
				<p className="text-xs font-mono text-matrix-muted">
					Sheets, dialogs, alert dialogs, popovers, and collapsible sections — all the layered
					surface patterns.
				</p>
			</AsciiBox>
			...
			<Section title="DIALOGS">
				<div className="flex flex-wrap gap-3">
					<MatrixButton size="small" onClick={() => setBasicDialog(true)}>
						BASIC
					</MatrixButton>
					<MatrixButton size="small" onClick={() => setCompactDialog(true)}>
						COMPACT
					</MatrixButton>
					<MatrixButton size="small" onClick={() => setScrollDialog(true)}>
						SCROLLABLE
					</MatrixButton>
				</div>
			</Section>
			...
			<Section title="POPOVERS">
				<div className="flex flex-wrap gap-3">
					<div>
						<MatrixButton ref={filterRef} size="small" onClick={() => setFilterPop(!filterPop)}>
							FILTER
						</MatrixButton>
						<FloatingPortal
						...
						>
							...
							<div className="flex gap-2 mt-3">
								<MatrixButton size="small" onClick={() => setFilterPop(false)}>
									RESET
								</MatrixButton>
								<MatrixButton size="small" primary onClick={() => setFilterPop(false)}>
									APPLY
								</MatrixButton>
							</div>
						</FloatingPortal>
					...
					<div>
						<MatrixButton ref={profileRef} size="small" onClick={() => setProfilePop(!profilePop)}>
							PROFILE
						</MatrixButton>
						<FloatingPortal
					...
					<div>
						<MatrixButton
							ref={settingsRef}
							size="small"
							onClick={() => setSettingsPop(!settingsPop)}
						>
							SETTINGS
						</MatrixButton>
						<FloatingPortal
			...
			<Sheet side={sheetSide ?? "right"} open={!!sheetSide} onClose={() => setSheetSide(null)}>
				{sheetSide === "right" && (
					<div className="space-y-4">
						...
						<div className="flex gap-2 pt-2">
							...
							<MatrixButton primary onClick={() => setSheetSide(null)}>
								SAVE
							</MatrixButton>
						</div>
					</div>
				...
				{sheetSide === "left" && (
					<div className="space-y-4">
						...
						<MatrixButton primary onClick={() => setSheetSide(null)} className="w-full">
							APPLY FILTERS
						</MatrixButton>
					</div>
				)}
				...
				{sheetSide === "bottom" && (
					<div>
						<p className="text-xs font-mono text-matrix-primary font-bold uppercase mb-3">
							Quick Actions
						</p>
					...
					<div className="fixed inset-x-4 top-[20%] z-50 mx-auto max-w-md">
						<AsciiBox title="EDIT PROFILE">
							<div className="space-y-3">
								...
								<div>
									<label className="text-xs font-mono text-matrix-muted uppercase font-bold block mb-2">
										Bio
										<textarea
											placeholder="Enter your bio..."
											rows={3}
											className="w-full mt-2 bg-matrix-panel border border-matrix-ghost px-3 py-2 text-sm font-mono text-matrix-bright outline-none focus:border-matrix-primary resize-none normal-case"
										/>
									</label>
								</div>
								<div className="flex justify-end gap-2 pt-2">
									<MatrixButton onClick={() => setBasicDialog(false)}>CANCEL</MatrixButton>
									<MatrixButton primary onClick={() => setBasicDialog(false)}>
										SAVE
									</MatrixButton>
								</div>
							</div>
			...
			{compactDialog && (
				<>
					<div
						className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm"
						onClick={() => setCompactDialog(false)}
					/>
					<div className="fixed inset-x-4 top-[25%] z-50 mx-auto max-w-sm">
						<div className="matrix-panel border border-matrix-primary/30 p-5">
							...
							<div className="flex justify-end gap-2">
								...
								<MatrixButton primary onClick={() => setCompactDialog(false)}>
									CONFIRM
								</MatrixButton>
							</div>
						</div>
						...
						<AsciiBox title="TERMS OF SERVICE">
							<div className="space-y-4 text-xs font-mono text-matrix-dim">
								{Array.from({ length: 8 }, (_, i) => (
									<div key={i}>
										<p className="text-matrix-muted font-bold mb-1">
											SECTION {i + 1} —{" "}
											{
												[
										...
										<p>
											By accessing the Matrix neural interface, you acknowledge and agree to the
											terms outlined in this section. All data transmitted through the interface is
											subject to encryption protocols and monitoring systems as defined by Zion
											Security Council directive ZSC-{2026 + i}. Unauthorized access attempts will
							...
							<div className="flex justify-end gap-2 pt-4 border-t border-matrix-ghost mt-4">
								...
								<MatrixButton primary onClick={() => setScrollDialog(false)}>
									ACCEPT
								</MatrixButton>
							</div>
						</AsciiBox>
			...
			{discardAlert && (
				<>
					...
					<div className="fixed inset-x-4 top-[25%] z-50 mx-auto max-w-sm">
						<div className="matrix-panel border border-matrix-primary/30 p-5">
							<p className="text-sm font-mono text-matrix-primary font-bold mb-2">
								DISCARD CHANGES?
							</p>
							...
							<div className="flex justify-end gap-2">
								...
								<MatrixButton primary onClick={() => setDiscardAlert(false)}>
									DISCARD
								</MatrixButton>
							</div>
						</div>
