...
function Pagination({
	...
	return (
		<div className="flex items-center gap-1">
			...
			{getPages().map((p, i) =>
				p === "..." ? (
					<span key={`ellipsis-${i}`} className="px-1 text-[10px] font-mono text-matrix-dim">
						...
					</span>
				) : (
					<button
					...
					<div>
						...
						<div className="flex gap-2">
							...
							<MatrixButton
								size="small"
								primary
								onClick={vm.stepperNext}
								disabled={vm.stepperIndex === 4}
							>
								NEXT »
							</MatrixButton>
							<MatrixButton size="small" onClick={vm.stepperReset}>
								RESET
							</MatrixButton>
						</div>
					</div>
						...
						<div className="space-y-0">
							{vm.stepperSteps.map((step, i) => (
								<div key={step.label} className="flex gap-3">
									...
									<div className="pb-4">
										<p className="text-xs font-mono text-matrix-bright font-bold uppercase">
											{step.label}
										</p>
