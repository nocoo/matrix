...
export default function RecordListPage() {
	...
	return (
		<div className="space-y-4">
			<p className="font-mono text-xs text-matrix-dim">{totalCount} records found</p>
			...
			<AsciiBox title="TRANSACTION LOG">
				<div className="space-y-0">
					...
					{records.map((record) => (
						...
						>
							<span className="col-span-4 text-matrix-muted truncate">{record.name}</span>
							<span className="col-span-2 text-matrix-dim truncate">{record.category}</span>
							<span className="col-span-2 text-matrix-dim text-[11px]">{record.date}</span>
							<span
								className={cn(
