				...
				<div className="space-y-0">
					...
					{netFlowData.map((d) => (
						...
						>
							...
							<span className="text-right text-matrix-primary">${d.inflow.toLocaleString()}</span>
							<span className="text-right text-red-400">${d.outflow.toLocaleString()}</span>
							<span className="text-right text-matrix-primary">${d.net.toLocaleString()}</span>
						</div>
					))}
				</div>
			</AsciiBox>
