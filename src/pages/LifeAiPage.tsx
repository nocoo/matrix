...
import { TrendChart, TrendMonitor } from "@/components/ui/DataVizComponents";
		...
		<div className="space-y-4">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-2">
					<MatrixButton size="small" onClick={goToPrevDay}>
						[&lt;]
					</MatrixButton>
					<span className="font-mono text-sm text-matrix-primary">{dateStr}</span>
					<MatrixButton size="small" onClick={goToNextDay}>
						[&gt;]
					</MatrixButton>
				</div>
			...
			<div className="grid grid-cols-2 gap-3">
				<AsciiBox title="ACTIVE EVENTS">
					<p className="font-mono text-lg text-matrix-primary text-center">{activeEventCount}</p>
				</AsciiBox>
				<AsciiBox title="TOTAL CALORIES">
					<p className="font-mono text-lg text-matrix-primary text-center">{totalCalories} kcal</p>
				</AsciiBox>
			</div>
					...
					{timeline.map((event) => (
						<div key={event.id} className="flex items-start gap-3 font-mono text-xs py-1">
							...
							<div className="min-w-0">
								...
								{event.subtitle && <span className="text-matrix-dim ml-2">{event.subtitle}</span>}
							</div>
						</div>
					))}
				</div>
					...
					{activityBreakdown.map((item) => (
						<div key={item.label} className="flex items-center gap-3 font-mono text-xs">
							...
							<div className="flex-1 h-3 bg-matrix-primary/10 overflow-hidden">
								<div className="h-full bg-matrix-primary" style={{ width: `${item.value}%` }} />
							</div>
