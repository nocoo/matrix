...
import { ConnectionStatus, Sparkline, TypewriterText } from "@/components/ui/MatrixExtras";
	...
	return (
		<div className="space-y-3">
						...
						{signalRows.map((sig) => (
							<div key={sig.label} className="flex items-center justify-between gap-2">
								<div className="flex flex-col min-w-0">
									<span className="font-mono text-[10px] text-matrix-dim uppercase">
										{sig.label}
									</span>
									<span
										className={cn(
							...
							{pixelHeatmap.map((cell) => (
								<span
									...
									style={{
										...
										boxShadow:
											cell.value >= pixelHeatmapMax ? "0 0 4px rgba(0,255,65,0.6)" : "none",
									}}
					...
					<AsciiBox key={goal.name} title={goal.name.toUpperCase()}>
						<div className="space-y-2.5">
							<div className="flex items-center justify-between">
								...
								>
									<ConnectionStatus status={goal.onTrack ? "STABLE" : "UNSTABLE"} />
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
						...
						{budgetRows.map((budget) => (
							<div key={budget.category} className="space-y-1">
								<div className="flex items-center justify-between font-mono text-xs">
									...
									<span className={cn(budget.overBudget ? "text-red-400" : "text-matrix-primary")}>
				...
				<TrendMonitor data={trendData} label="30-DAY TREND" color="#00FF41" />
			</div>
			...
			<AsciiBox title="CASH FLOW">
				<div className="space-y-2">
					{flowData.map((flow) => {
						const maxVal = Math.max(...flowData.map((f) => Math.max(f.inflow, f.outflow)));
						return (
							<div key={flow.month} className="space-y-0.5">
								<div className="flex items-center justify-between font-mono text-[10px]">
									<span className="text-matrix-dim uppercase w-8">{flow.month}</span>
									<span className={cn(flow.net >= 0 ? "text-matrix-primary" : "text-red-400")}>
