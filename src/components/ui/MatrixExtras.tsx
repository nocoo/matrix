...
function scrambleValue(text: string, chars: string): string {
	const safeChars = chars?.length ? chars : DEFAULT_CHARS;
	return String(text)
		.split("")
	...
	return (
		<div className={`relative ${className}`}>
			...
			>
				...
				<span
					className={`text-matrix-dim text-caption transition-transform ${open ? "rotate-180" : ""}`}
				>
					▼
				</span>
			</button>
...
export function LeaderboardRow({
	...
	return (
		...
		>
			<div className="flex items-center space-x-3">
				<span
					className={`text-caption w-6 ${rankValue <= 3 ? "text-matrix-primary font-bold" : "text-matrix-muted"}`}
				>
					{formattedRank}
				</span>
...
export function LiveSniffer() {
	...
	const [logs, setLogs] = useState([
		"[SYSTEM] Live sniffer initialized",
...
export function FloatingPortal({
	...
	return createPortal(
		<>
			...
			<div className={`fixed z-[9999] ${className}`} style={style}>
				{children}
			</div>
		</>,
		document.body,
