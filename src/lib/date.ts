 ...
 */
export function formatScheduleTime(hour: number | string, minute: number | string): string {
	const h = hour === "*" ? "*" : typeof hour === "string" ? hour : String(hour).padStart(2, "0");
	const m =
		minute === "*" ? "*" : typeof minute === "string" ? minute : String(minute).padStart(2, "0");
	return `${h}:${m}`;
}
