...
describe("LeaderboardRow", () => {
	...
	it("applies custom className", () => {
		const { container } = render(<LeaderboardRow rank={1} name="X" value="1" className="lb-cls" />);
...
describe("MatrixSelect", () => {
	...
	it("renders label when provided", () => {
		render(<MatrixSelect label="Select Item" value="a" onChange={() => {}} options={options} />);
		expect(screen.getByText("Select Item")).toBeInTheDocument();
	});

	it("does not render label when not provided", () => {
		const { container } = render(<MatrixSelect value="a" onChange={() => {}} options={options} />);
		// The label span with specific classes should not exist
		const labelSpan = container.querySelector(
			"span.text-caption.text-matrix-muted.uppercase.font-bold.block.mb-2",
		);
	...
	it("handles scroll and resize events when open", () => {
		...
		const { unmount } = render(<MatrixSelect value="a" onChange={() => {}} options={options} />);
