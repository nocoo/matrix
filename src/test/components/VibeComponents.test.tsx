...
describe("IdentityCard", () => {
	...
	it("does not show stats when no rankLabel and no streakDays", () => {
		render(<IdentityCard name="Neo" isPublic showStats animate={false} animateTitle={false} />);
...
describe("TopModelsPanel", () => {
	...
	it("fills remaining slots when fewer than 3 rows provided", () => {
		render(<TopModelsPanel rows={[{ name: "GPT-4", percent: "100" }]} />);
		expect(screen.getByText("GPT-4")).toBeInTheDocument();
		expect(screen.getByText("01")).toBeInTheDocument();
		expect(screen.getByText("02")).toBeInTheDocument();
		expect(screen.getByText("03")).toBeInTheDocument();
...
describe("LeaderboardPanel", () => {
	...
	it("shows summary view when period matches summaryPeriod", () => {
		...
		render(
			<LeaderboardPanel rows={sampleRows} period="ALL" summaryPeriod="ALL" summary={summary} />,
		);
...
describe("UsagePanel", () => {
	...
	it("hides header when hideHeader is true", () => {
		render(<UsagePanel hideHeader periods={[{ key: "24H", label: "24H" }]} onRefresh={() => {}} />);
	...
	it("renders summarySubLabel", () => {
		render(<UsagePanel showSummary summaryAnimate={false} summarySubLabel="last 30 days" />);
...
describe("NeuralAdaptiveFleet", () => {
	...
	it("renders model legend items", () => {
		render(<NeuralAdaptiveFleet label="CODING" totalPercent={100} models={sampleModels} />);
		expect(screen.getByText("GPT-4")).toBeInTheDocument();
				...
				fleetData={[
					{
						label: "FLEET",
						usd: 10,
						models: [{ id: null as unknown as string, name: "ModelName", share: 30 }],
					},
				]}
			/>,
		);
				...
				fleetData={[
					{
						label: "FLEET",
						usd: 5,
						models: [{ id: "m1", name: "Model1", share: 25, calc: "" }],
					},
				]}
			/>,
		);
				...
				fleetData={[
					{
						label: "FLEET",
						usd: 10,
						models: [{ id: "m1", name: "ModelInf", share: Infinity }],
					},
				]}
			/>,
		);
				...
				fleetData={[
					{
						label: "FLEET",
						usd: 10,
						models: [{ id: "m1", name: "ModelNaN", share: NaN }],
					},
				]}
			/>,
		);
...
describe("VibeComponents edge branches", () => {
	it("IdentityPanel falls back when streakDays is non-finite", () => {
		const { container } = render(
			<IdentityPanel name="alice" streakDays={NaN as unknown as number} />,
		);
	...
	it("TopModelsPanel handles row missing name/percent (fallback dash)", () => {
		render(<TopModelsPanel rows={[{ id: "a", name: "", percent: "" }]} />);
	...
	it("CostAnalysisModal: non-array fleetData is treated as empty", () => {
		render(
			<CostAnalysisModal isOpen onClose={() => {}} fleetData={undefined as unknown as never} />,
		);
	...
	it("CostAnalysisModal: onClose not a function is a no-op (early return branch)", () => {
		const { container } = render(
			<CostAnalysisModal isOpen onClose={undefined as unknown as () => void} fleetData={[]} />,
		);
		fireEvent.keyDown(window, { key: "Escape" });
		expect(container).toBeInTheDocument();
	});
				...
				fleetData={[
					{
						label: "F",
						usd: 1,
						models: [{ name: "m", share: NaN as unknown as number, calc: "  custom  " }],
					},
				]}
			/>,
		);
